...
import { BaseRunner } from '../typeorm/runner/base-runner';
import { runnerFactory } from '../typeorm/runner/runner-factory';
import {
  FilterGeoBody,
  GetCountGroupParam,
  GISCrudRequest,
...
export class GISTypeOrmCrudService<T> extends BaseTypeOrmCrudService<T> {
  ...
  protected async setAndWhereBBox(
    ...
    if (wktGeo) {
      const alias = pAlias ? pAlias + '.' : '';
      const where = this.runner.spatialQuery({
        wktGeo,
        alias,
        propertyName: geoColumn.propertyName,
      });
      if (builder.getSql().search('WHERE') === -1) builder.where(where);
      else builder.andWhere(where);
    }
    ...
    if (shape) {
      ...
      if (!this.equalSrs(req.parsed.inSR, moduleOptions.srs)) {
        ...
        if (geometries.length) {
          shape = arcgis.parse(geometries[0]);
        } else {
  ...
  private async getPrimaryCol() {
    ...
    try {
      isGenerated = await this.runner.getGeneratedColumn({
        column: primaryCol.databaseName,
      });
      if (isGenerated) {
  ...
  async createMany(req: GISCrudRequest, dto: CreateManyDto<DeepPartial<T>>) {
    ...
    if (hasGeoData) {
      ...
      if (!this.equalSrs(req.parsed.inSR, moduleOptions.srs)) {
        ...
        geometries.forEach(
          (geo, idx) => (bulk[idx]['shape'] = arcgis.parse(geo)),
        );
      }
  ...
  async updateOne(req: GISCrudRequest, dto: DeepPartial<T>) {
    ...
    if (shape) {
      ...
      if (!this.equalSrs(req.parsed.inSR, moduleOptions.srs)) {
        ...
        if (geometries.length) {
          dto[geoColumn.propertyName] = arcgis.parse(geometries[0]);
        } else {
          throw new Error('Không thể chuyển hệ tọa độ, vui lòng thử lại');
        }
      }
